import { signOut } from 'firebase/auth';
import React from 'react'
import { useNavigate } from 'react-router-dom'
import { auth } from '../utils/firebase';


const Browse = () => {
  const navigate = useNavigate();
  
  const handleEventSignOut = () => {
    signOut(auth).then(() => {
      navigate("/")
    }).catch((error) => {
      navigate('/error')
    });
  }

  return (
    <>
    <div className='absolute w-screen pl-10 bg-gradient-to-b from-black z-30 flex justify-between'>
        <img
        className='w-36 p-2 ' 
        src="https://cdn.cookielaw.org/logos/dd6b162f-1a32-456a-9cfe-897231c7763c/4345ea78-053c-46d2-b11e-09adaef973dc/Netflix_Logo_PMS.png" 
        alt="Netflix"
         />

<div className='flex px-8 items-center'>
    <img
        className='w-16 h-16 p-2 rounded-xl' 
        src="data:image/png;base64," 
        alt="logo"
         />
      <button onClick={handleEventSignOut}className='font-bold text-white bg-red-800 hover:bg-red-600 w-24 h-8 border rounded-lg border-black'>Sign Out</button>
    </div>
    </div>
    
    </>
  )
}

export default Browse